...
if (form && queryInput && resultsList && emptyState && resultsCount) {
    form.addEventListener("submit", async (event) => {
        ...
        const nextUrl = query ? `/v2?q=${encodeURIComponent(query)}` : "/v2";
        ...
        if (!response.ok) {
            renderResults([]);
            emptyState.textContent = "Semantic search failed. Please try again.";
...
function renderResults(results) {
    ...
    if (results.length === 0) {
        emptyState.textContent = "No matching semantic results found.";
        emptyState.classList.remove("hidden");
        return;
    }
